...
# JIT-compiled dispatching rule kernels
@njit(cache=True)
def _spt_rule(jobs: int, legal_actions: np.ndarray, todo_time_step_job: np.ndarray, 
              instance_matrix: np.ndarray) -> int:
    """JIT-compiled SPT rule."""
    min_time = 1e9
    min_job = -1
    for job in range(jobs):
        if legal_actions[job]:
            process_time = instance_matrix[job, todo_time_step_job[job], 1]
...
@njit(cache=True)
def _fifo_rule(jobs: int, legal_actions: np.ndarray, idle_time_jobs_last_op: np.ndarray) -> int:
    """JIT-compiled FIFO rule."""
    max_idle = -1
    max_job = -1
    for job in range(jobs):
        if legal_actions[job]:
            if idle_time_jobs_last_op[job] > max_idle:
                max_idle = idle_time_jobs_last_op[job]
                max_job = job
    return max_job


@njit(cache=True)
def _mwr_rule(jobs: int, machines: int, legal_actions: np.ndarray, 
              todo_time_step_job: np.ndarray, instance_matrix: np.ndarray) -> int:
    """JIT-compiled MWR rule."""
    max_remaining = -1
    max_job = -1
    for job in range(jobs):
        if legal_actions[job]:
            ...
            for op in range(todo_time_step_job[job], machines):
                remaining += instance_matrix[job, op, 1]
...
@njit(cache=True)
def _lwr_rule(jobs: int, machines: int, legal_actions: np.ndarray,
              todo_time_step_job: np.ndarray, instance_matrix: np.ndarray) -> int:
    """JIT-compiled LWR rule."""
    min_remaining = 1e9
    min_job = -1
    for job in range(jobs):
        if legal_actions[job]:
...
@njit(cache=True)
def _mor_rule(jobs: int, machines: int, legal_actions: np.ndarray,
              todo_time_step_job: np.ndarray) -> int:
    """JIT-compiled MOR rule."""
    max_ops = -1
    max_job = -1
    for job in range(jobs):
        if legal_actions[job]:
            remaining_ops = machines - todo_time_step_job[job]
            if remaining_ops > max_ops:
...
@njit(cache=True)
def _lor_rule(jobs: int, machines: int, legal_actions: np.ndarray,
              todo_time_step_job: np.ndarray) -> int:
    """JIT-compiled LOR rule."""
    min_ops = 1000000
    min_job = -1
    for job in range(jobs):
        if legal_actions[job]:
            remaining_ops = machines - todo_time_step_job[job]
            if remaining_ops < min_ops:
                min_ops = remaining_ops
                min_job = job
    return min_job
...
class ShortestProcessingTime(DispatchingRule):
    ...
    def __call__(self, env: JssEnv) -> int:
        ...
        # If waiting (no-op) is the only legal action (optimized check)
        if env.nb_legal_actions == 0:
            return env.jobs
        
        # Use JIT-compiled kernel
        result = _spt_rule(env.jobs, legal_actions, env.todo_time_step_job, env.instance_matrix)
        
        # Random exploration (disabled by default for optimization)
        if self.exploration_rate > 0 and legal_actions[env.jobs] and np.random.random() < self.exploration_rate:
            return env.jobs
...
class FirstInFirstOut(DispatchingRule):
    ...
    def __call__(self, env: JssEnv) -> int:
        ...
        # Use JIT-compiled kernel
        result = _fifo_rule(env.jobs, legal_actions, env.idle_time_jobs_last_op)
...
class MostWorkRemaining(DispatchingRule):
    ...
    def __call__(self, env: JssEnv) -> int:
        ...
        # Use JIT-compiled kernel
        result = _mwr_rule(env.jobs, env.machines, legal_actions, env.todo_time_step_job, env.instance_matrix)
...
class LeastWorkRemaining(DispatchingRule):
    ...
    def __call__(self, env: JssEnv) -> int:
        ...
        # Use JIT-compiled kernel
        result = _lwr_rule(env.jobs, env.machines, legal_actions, env.todo_time_step_job, env.instance_matrix)
...
class MostOperationsRemaining(DispatchingRule):
    ...
    def __call__(self, env: JssEnv) -> int:
        ...
        # Use JIT-compiled kernel
        result = _mor_rule(env.jobs, env.machines, legal_actions, env.todo_time_step_job)
...
class LeastOperationsRemaining(DispatchingRule):
    ...
    def __call__(self, env: JssEnv) -> int:
        ...
        # Use JIT-compiled kernel
        result = _lor_rule(env.jobs, env.machines, legal_actions, env.todo_time_step_job)
